...
WEBSOCKET_RECEIVE_TIMEOUT = int(
    os.environ.get("WEBSOCKET_RECEIVE_TIMEOUT", 30)
)  # seconds
MAX_EXECUTION_TIME = int(
    os.environ.get("MAX_EXECUTION_TIME", 1200)
)  # 20 minutes default
...
def callback_api(payload):
    ...
    if CALLBACK_API_ENDPOINT != "":
        try:
            ...
            response = requests.post(
                CALLBACK_API_ENDPOINT, json=payload, headers=headers, timeout=30
            )
            if response.status_code != 200:
...
def file_handler(job_id, node_id, execution_time, file_info):
    ...
    if not filename:
        print(
            f"worker-comfyui - Skipping image in node {node_id} due to missing filename: {file_info}"
        )
        return None
    ...
    try:
        ...
        while True:
            ...
            if current_time - start_time > MAX_EXECUTION_TIME:
                raise TimeoutError(
                    f"Job execution exceeded maximum time limit of {MAX_EXECUTION_TIME} seconds"
                )
            ...
            try:
                ...
                if isinstance(out, str):
                    ...
                    if message.get("type") == "status":
                        ...
                        if data.get("prompt_id") == prompt_id:
                            ...
                            node_id = data.get("node")
                            print(
                                f"worker-comfyui - Progress: {value}/{max_val} (Node: {node_id})"
                            )
                ...
                if elapsed > 120:  # 2 minutes without any message
                    print(
                        f"worker-comfyui - No messages received for {elapsed:.1f}s, checking server status..."
                    )
                    srv_status = _comfy_server_status()
                    if not srv_status["reachable"]:
                        raise ConnectionError(
                            "ComfyUI server became unreachable during execution"
                        )
                print(
                    f"worker-comfyui - Websocket receive timed out. Still waiting... (elapsed: {current_time - start_time:.1f}s)"
                )
                continue
